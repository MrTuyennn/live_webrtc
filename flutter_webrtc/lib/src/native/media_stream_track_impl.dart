...
class MediaStreamTrackNative extends MediaStreamTrack {
  ...
  @override
  Future<ByteBuffer> captureFrame(String filePathName) async {
    // final filePath = await getTemporaryDirectory();
    //final String filePathName =
    //    filePath.path + '/' + _trackId + 'captureFrame.png';

    await WebRTC.invokeMethod(
      'captureFrame',
      <String, dynamic>{'trackId': _trackId, 'path': filePathName},
    );
    return File(filePathName).readAsBytes().then((value) => value.buffer);
  }
