...
import static com.rhrtc.flutter_webrtc.utils.MediaConstraintsUtils.parseMediaConstraints;
...
import com.rhrtc.flutter_webrtc.record.AudioChannel;
import com.rhrtc.flutter_webrtc.record.FrameCapturer;
import com.rhrtc.flutter_webrtc.utils.AnyThreadResult;
import com.rhrtc.flutter_webrtc.utils.Callback;
import com.rhrtc.flutter_webrtc.utils.ConstraintsArray;
import com.rhrtc.flutter_webrtc.utils.ConstraintsMap;
import com.rhrtc.flutter_webrtc.utils.EglUtils;
import com.rhrtc.flutter_webrtc.utils.ObjectType;
import com.rhrtc.flutter_webrtc.utils.PermissionUtils;
...
public class MethodCallHandlerImpl implements MethodCallHandler, StateProvider {
  ...
  private final List<String> streamIds = new ArrayList<String>();
...
/* 
  ...
  public String peerConnectionInit(ConstraintsMap configuration, ConstraintsMap constraints) {
    ...
    // Log.w(TAG, "peerConnectionInit()-------------"+peerConnectionId);
  ...
  public boolean CheckStreamId(String id) {
       for(int i = 0;i< streamIds.size();i++){
             if(streamIds.get(i).equals(id)){
                  return true;
             }
  ...
  public String getNextStreamUUID() {
    ...
    } while (getStreamForId(uuid, "") != null || CheckStreamId(uuid) == true);
    streamIds.add(uuid);
    return uuid;
  }


  public boolean CheckTrackId(String id) {
       for(int i = 0;i< trackIds.size();i++){
             if(trackIds.get(i).equals(id)){
                  return true;
             }
  ...
  public String getNextTrackUUID() {
    ...
    do {
      uuid = UUID.randomUUID().toString();
    } while (getTrackForId(uuid) != null ||CheckTrackId(uuid) == true);
    trackIds.add(uuid);
    return uuid;
  }
