...
- (instancetype)initWithChannel:(FlutterMethodChannel *)channel
   ...
   //                                                                                                      fallback:encoderFactory];
    ...
    RTCSetMinDebugLogLevel(RTCLoggingSeverityWarning);//RTCLoggingSeverityWarning,RTCLoggingSeverityInfo
...
- (void)handleMethodCall:(FlutterMethodCall*)call result:(FlutterResult) result {
    ...
    if ([@"createPeerConnection" isEqualToString:call.method]) {
        ...
        NSString* peerConnectionId = argsMap[@"peerConnectionId"];   
       int audioChannel = ((NSNumber *)argsMap[@"audioChannel"]).intValue;
        if (videoTrackId == nil) {
            result([FlutterError errorWithCode:@"video track is nil" message:nil details:nil]);
            return;
        }
        
        RTCMediaStreamTrack *videoTrack = [self trackForId: videoTrackId];
        if (![videoTrack isKindOfClass:[RTCVideoTrack class]]) {
            result([FlutterError errorWithCode:[@"Track is class of " stringByAppendingString:[[videoTrackId class] description]] message:nil details:nil]);
            return;
        }
       ...
       if (audioTrack == nil) {
            NSLog(@"startRecordToFile audioTrack is null");
       }else{
//          NSLog(@"startRecordToFile  ");
       }
        [self mediaStreamTrackStartRecord:(RTCVideoTrack *)videoTrack
                               audioTrack:audioTrack
                                   toPath:path
                                   result:result];
    } else if ([@"stopRecordToFile" isEqualToString:call.method]) {
//        NSDictionary* argsMap = call.arguments;
//        int recorderId = ((NSNumber *)argsMap[@"recorderId"]).intValue;
        [self mediaStreamTrackStopRecord:result];     
//        result(FlutterMethodNotImplemented);
